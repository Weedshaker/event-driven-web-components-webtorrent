  ...
  constructor() {
    ...
    */
    ...
    this.ipfsAddEventListener = async event => {
      ...
      event.detail.torrent.on('ready', () => addWebSeedFunc(event.detail.torrent))
    }
    ...
    this.ipfsSeedEventListener = async event => {
      ...
      for await (const result of client.addAll(Array.from(event.detail.input).map(file => ({
        path: file.name,
        content: file // passing file.stream() does not work, for that reason we wait for torrent on done and not torrent on stream to pass the files for seeding
      })), {
        pin: true, 
